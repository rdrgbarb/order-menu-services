...
public class OrderServiceImpl implements OrderService {
    ...
    public Order create(CreateOrderRequest request) {
        ...
        try {
            items = request.orderItems().stream().map(item -> {
                MenuClient.MenuItem menuItem = menuClient.getMenuItem(item.productId()).
                    orElseThrow(() -> badRequest("Invalid product ID: " + item.productId()));

                return new OrderItem(item.productId(), menuItem.name(), menuItem.price(), item.quantity());
            }).toList();
